...
A Session surface outside the transcript reads where the mounted Chat view is through `ctx.chatView.location(sessionId)`: a source stable per Session reporting the Turn at the reading line and the Turn a jump is currently landing on. These values are view state — they follow the reader's scroll position and the jump lifecycle, never the event log — and the view is their only writer. A Session whose Chat view is unmounted reports both as `null`, so a consumer never highlights a position nothing is showing. The service is optional: `ctx.get('chatView')` is `undefined` when no Chat plugin is mounted.
