...
- **不做虚拟化。**每个节点都留在 DOM 里。链上带 `content-visibility: auto`：它包含整条链，并在面板未显示时跳过链的内容；地图在屏幕上时链整体渲染，所以读者看着地图时的行成本并没有被削减。轮次数由会话决定，而不是由产品决定；触发虚拟列表所需的实测值——滚动或提交成本开始不可接受时的行数——没有记录。
